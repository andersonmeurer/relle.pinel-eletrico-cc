...
typedef struct tskTaskControlBlock
{
	...
	signed portCHAR			pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */
		...
		unsigned int uxCriticalNesting;
	#endif

	#if ( configUSE_TRACE_FACILITY == 1 )
		unsigned int	uxTCBNumber;		/*< This is used for tracing the scheduler and making debugging easier only. */
	#endif	
...
static xList xPendingReadyList;							/*< Tasks that have been readied while the scheduler was suspended.  They will be moved to the ready queue when the scheduler is resumed. */
	...
	static volatile unsigned int uxTasksDeleted = ( unsigned int ) 0;
...
static volatile unsigned int uxCurrentNumberOfTasks	= ( unsigned int ) 0;
static volatile portTickType xTickCount							= ( portTickType ) 0;
static unsigned int uxTopUsedPriority					= tskIDLE_PRIORITY;
static volatile unsigned int uxTopReadyPriority		= tskIDLE_PRIORITY;
static volatile signed int xSchedulerRunning			= pdFALSE;
static volatile unsigned int uxSchedulerSuspended		= ( unsigned int ) pdFALSE;
static volatile unsigned int uxMissedTicks			= ( unsigned int ) 0;
static volatile int xMissedYield						= ( int ) pdFALSE;
static volatile int xNumOfOverflows					= ( int ) 0;
static unsigned int uxTaskNumber = 0;
 ...
 */
	...
	static signed int xTracing = pdFALSE;
	static unsigned int uxPreviousTask = 255;
 ...
 */
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed portCHAR * const pcName, unsigned int uxPriority );
...
signed int xTaskCreate( pdTASK_CODE pvTaskCode, const signed portCHAR * const pcName, unsigned portSHORT usStackDepth, void *pvParameters, unsigned int uxPriority, xTaskHandle *pxCreatedTask )
{
signed int xReturn;
tskTCB * pxNewTCB;
	...
	{		
		...
		portENTER_CRITICAL();
		{
			...
			if( uxCurrentNumberOfTasks == ( unsigned int ) 1 )
			{
 ...
 *----------------------------------------------------------*/
	...
	int xAlreadyYielded, xShouldDelay = pdFALSE;
...
/*-----------------------------------------------------------*/
	...
	{
	portTickType xTimeToWake;
	signed int xAlreadyYielded = pdFALSE;
...
/*-----------------------------------------------------------*/
	...
	unsigned int uxTaskPriorityGet( xTaskHandle pxTask )
	{
	tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
	...
	void vTaskPrioritySet( xTaskHandle pxTask, unsigned int uxNewPriority )
	{
	tskTCB *pxTCB;
	unsigned int uxCurrentPriority, xYieldRequired = pdFALSE;
...
/*-----------------------------------------------------------*/
	...
	signed int xTaskIsTaskSuspended( xTaskHandle xTask )
	{
	int xReturn = pdFALSE;
...
/*-----------------------------------------------------------*/
	...
	int xTaskResumeFromISR( xTaskHandle pxTaskToResume )
	{
	...
	tskTCB *pxTCB;
		...
		if( xTaskIsTaskSuspended( pxTCB ) == pdTRUE )
		{
			...
			if( uxSchedulerSuspended == ( unsigned int ) pdFALSE )
			{
...
signed int xTaskResumeAll( void )
{
register tskTCB *pxTCB;
signed int xAlreadyYielded = pdFALSE;
	...
	portENTER_CRITICAL();
	{
		...
		if( uxSchedulerSuspended == ( unsigned int ) pdFALSE )
		{			
			if( uxCurrentNumberOfTasks > ( unsigned int ) 0 )
			{
				int xYieldRequired = pdFALSE;
				...
				if( uxMissedTicks > ( unsigned int ) 0 )
				{
					while( uxMissedTicks > ( unsigned int ) 0 )
					{
						vTaskIncrementTick();
						--uxMissedTicks;
					}
...
/*-----------------------------------------------------------*/
	...
	unsigned int uxQueue;
        ...
        vTaskSuspendAll();
		{
			...
			strcat( ( portCHAR * ) pcWriteBuffer, ( const portCHAR * ) "\r\r\n" );
...
/*-----------------------------------------------------------*/
	...
	int xTaskCallApplicationTaskHook( xTaskHandle xTask, void *pvParameter )
	{
	tskTCB *xTCB;
	int xReturn;
...
signed int xTaskRemoveFromEventList( const xList * const pxEventList )
{
...
int xTaskCheckForTimeOut( xTimeOutType * const pxTimeOut, portTickType * const pxTicksToWait )
{
int xReturn;
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		...
		#if ( ( configUSE_PREEMPTION == 1 ) && ( configIDLE_SHOULD_YIELD == 1 ) )
		{
			...
			if( listCURRENT_LIST_LENGTH( &( pxReadyTasksLists[ tskIDLE_PRIORITY ] ) ) > ( unsigned int ) 1 )
			{
				taskYIELD();
			}
		}
...
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed portCHAR * const pcName, unsigned int uxPriority )
{
	...
	#if ( portCRITICAL_NESTING_IN_TCB == 1 )
	{
		pxTCB->uxCriticalNesting = ( unsigned int ) 0;
	}
	#endif
...
{							
	...
	{				
		...
		if( uxTasksDeleted > ( unsigned int ) 0 )
		{
			vTaskSuspendAll();
...
/*-----------------------------------------------------------*/
	...
	unsigned portSHORT usStackRemaining;
		...
		do
		{
			...
			sprintf( pcStatusString, ( portCHAR * ) "%s\t\t%c\t%u\t%u\t%u\r\r\n", pxNextTCB->pcTaskName, cStatus, ( unsigned int ) pxNextTCB->uxPriority, usStackRemaining, ( unsigned int ) pxNextTCB->uxTCBNumber );
...
/*-----------------------------------------------------------*/
	...
	unsigned int uxTaskGetStackHighWaterMark( xTaskHandle xTask )
	{
	tskTCB *pxTCB;
