...
typedef struct tskTaskControlBlock
{
	...
	char				pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */ /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
		...
		u32		ulRunTimeCounter;	/*< Stores the amount of time the task has spent in the Running state. */
	#endif
...
PRIVILEGED_DATA static volatile UBaseType_t uxSchedulerSuspended	= ( UBaseType_t ) pdFALSE;
	...
	PRIVILEGED_DATA static u32 ulTaskSwitchedInTime = 0UL;	/*< Holds the value of a timer/counter the last time a task was switched in. */
	PRIVILEGED_DATA static u32 ulTotalRunTime = 0UL;		/*< Holds the total amount of execution time as defined by the run time counter clock. */
 ...
 */
	...
	static uint16_t prvTaskCheckFreeStackSpace( const u8* pucStackByte ) PRIVILEGED_FUNCTION;
...
/*-----------------------------------------------------------*/
	...
	UBaseType_t uxTaskGetSystemState( TaskStatus_t * const pxTaskStatusArray, const UBaseType_t uxArraySize, u32 * const pulTotalRunTime )
	{
	...
	UBaseType_t uxTask = 0;
		...
		if( listCURRENT_LIST_LENGTH( pxList ) > ( UBaseType_t ) 0 )
		{
			...
			do
			{
				...
				#if ( portSTACK_GROWTH > 0 )
				{
					pxTaskStatusArray[ uxTask ].usStackHighWaterMark = prvTaskCheckFreeStackSpace( ( u8* ) pxNextTCB->pxEndOfStack );
				}
				#else
				{
					pxTaskStatusArray[ uxTask ].usStackHighWaterMark = prvTaskCheckFreeStackSpace( ( u8* ) pxNextTCB->pxStack );
				}
				#endif
...
/*-----------------------------------------------------------*/
	...
	static uint16_t prvTaskCheckFreeStackSpace( const u8* pucStackByte )
	{
	u32 ulCount = 0U;

		while( *pucStackByte == ( u8) tskSTACK_FILL_BYTE )
		{
			pucStackByte -= portSTACK_GROWTH;
			ulCount++;
		}

		ulCount /= ( u32 ) sizeof( StackType_t ); /*lint !e961 Casting is not redundant on smaller architectures. */
...
/*-----------------------------------------------------------*/
	...
	UBaseType_t uxReturn;
		...
		#if portSTACK_GROWTH < 0
		{
			pucEndOfStack = ( u8* ) pxTCB->pxStack;
		}
		#else
		{
			pucEndOfStack = ( u8* ) pxTCB->pxEndOfStack;
		}
		#endif
...
/*-----------------------------------------------------------*/
	...
	u32 ulTaskNotifyTake( BaseType_t xClearCountOnExit, TickType_t xTicksToWait )
	{
	TickType_t xTimeToWake;
	u32 ulReturn;
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xTaskNotifyWait( u32 ulBitsToClearOnEntry, u32 ulBitsToClearOnExit, u32 *pulNotificationValue, TickType_t xTicksToWait )
	{
	TickType_t xTimeToWake;
	BaseType_t xReturn;
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xTaskGenericNotify( TaskHandle_t xTaskToNotify, u32 ulValue, eNotifyAction eAction, u32 *pulPreviousNotificationValue )
	{
	TCB_t * pxTCB;
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xTaskGenericNotifyFromISR( TaskHandle_t xTaskToNotify, u32 ulValue, eNotifyAction eAction, u32 *pulPreviousNotificationValue, BaseType_t *pxHigherPriorityTaskWoken )
	{
	TCB_t * pxTCB;
	eNotifyValue eOriginalNotifyState;
	BaseType_t xReturn = pdPASS;
